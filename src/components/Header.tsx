...
const SmartNavbar: React.FC = () => {
  ...
  // Framer Motion variants for smooth sliding with added entrance animation
  const variants = {
  ...
  // Stagger variants for navigation links
  const linkVariants = {
  ...
  return (
    ...
    >
      <div className="bg-purple-600 px-3 py-2 md:px-6 md:py-3 flex justify-between items-center h-14 md:h-16   transition-shadow duration-300">
        {/* Left Side: Logo/Brand with glow effect */}
        <motion.div
        ...
        {/* Center: Navigation Links with stagger animation */}
        <motion.div 
        ...
        >
          <motion.div variants={linkVariants}>
            <Link 
              ...
              className={`text-white/90 hover:text-white transition text-sm lg:text-base font-medium ${
                location.pathname === '/' ? 'bg-white/20 rounded-lg px-2 py-1' : ''
              }`}
            >
              Home
            </Link>
          ...
          <motion.div variants={linkVariants}>
            <Link 
              to="/events" 
              className={`text-white/90 hover:text-white transition text-sm lg:text-base font-medium ${
                location.pathname === '/events' ? 'bg-white/20 rounded-lg px-2 py-1' : ''
              }`}
            >
              Events
            </Link>
        ...
        {/* Right Side: User Menu or Login with enhanced animations */}
        <div className="flex items-center space-x-2 md:space-x-3">
          {isAuthenticated ? (
            <div className="relative">
              ...
              {showUserMenu && (
                <motion.div 
                  className="absolute right-0 mt-2 w-44 md:w-48 bg-white rounded-lg shadow-xl py-1 md:py-2 z-50"
